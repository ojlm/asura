...
case class KarateCommandRunner(
  ...
  override def beforeScenario(scenario: Scenario, context: ScenarioContext): Boolean = {
    if (logActor != null) logActor ! DriverCommandLog(Commands.KARATE, "scenario", s"${scenario.getName} start")
    continue()
  }

  override def afterScenario(result: ScenarioResult, context: ScenarioContext): Unit = {
    if (logActor != null) logActor ! DriverCommandLog(Commands.KARATE, "scenario", s"${result.getScenario.getName} end")
  }
  ...
  override def afterAll(results: Results): Unit = {
    if (logActor != null && results != null) {
      logActor ! DriverCommandLog(Commands.KARATE, "result", KarateCommandRunner.resultsToString(results))
    }
  }
