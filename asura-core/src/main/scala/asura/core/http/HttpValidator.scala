...
object HttpValidator {
  ...
  def check(c: HttpCaseRequest, ns: String): ErrorMessages.ErrorMessage = {
  ...
  def check(c: HttpCaseRequest): ErrorMessages.ErrorMessage = {
    if (null == c) {
  ...
  def check(cs: Seq[HttpCaseRequest]): ErrorMessages.ErrorMessage = {
