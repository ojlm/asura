...
object CiTriggerService extends CommonService {
  ...
  def index(doc: CiTrigger): Future[IndexDocResponse] = {
    ...
    if (null == errorMsg) {
      EsClient.esClient.execute {
        indexInto(CiTrigger.Index / EsConfig.DefaultType).doc(doc).refresh(RefreshPolicy.WaitFor)
  ...
  def deleteTrigger(id: String): Future[DeleteDocResponse] = {
    EsClient.esClient.execute {
      delete(id).from(CiTrigger.Index / EsConfig.DefaultType).refresh(RefreshPolicy.WaitFor)
