...
object SqlRequestService extends CommonService with BaseAggregationService {
  ...
  def index(doc: SqlRequest): Future[IndexDocResponse] = {
    ...
    if (null == error) {
      EsClient.esClient.execute {
        indexInto(SqlRequest.Index / EsConfig.DefaultType).doc(doc).refresh(RefreshPolicy.WaitFor)
  ...
  def deleteDoc(id: String): Future[DeleteDocResponse] = {
    if (StringUtils.isEmpty(id)) {
      ...
      EsClient.esClient.execute {
        delete(id).from(SqlRequest.Index / EsConfig.DefaultType).refresh(RefreshPolicy.WaitFor)
  ...
  def deleteDoc(ids: Seq[String]): Future[BulkDocResponse] = {
    if (null == ids || ids.isEmpty) {
      ...
      EsClient.esClient.execute {
        bulk(ids.map(id => delete(id).from(SqlRequest.Index / EsConfig.DefaultType)))
