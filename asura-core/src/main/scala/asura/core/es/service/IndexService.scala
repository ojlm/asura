...
import asura.core.es.model.{FieldKeys, IndexSetting, JobReportDataHttpItem, RestApiOnlineLog}
...
object IndexService extends CommonService {
  ...
  def checkTemplate(): Boolean = {
    checkIndexTemplate(JobReportDataHttpItem).await && checkIndexTemplate(RestApiOnlineLog).await
  }
